...
│  • Mercado de fichajes              │
│  • Nuevos talentos (16-18 años)     │
│  • Pilotos procedurales             │
└──────────────┬──────────────────────┘
               │
               ▼
┌─────────────────────────────────────┐
│  ASIGNAR CARACTERÍSTICAS FÍSICAS    │
│  • Altura: 155-210cm                │
│  • Peso: 55-100kg                   │
│  • Apariencia: Pelo, ojos, piel     │
│  • Nacionalidad → Rasgos faciales   │
└──────────────┬──────────────────────┘
               │
               ▼
┌─────────────────────────────────────┐
│  GENERAR MODELO 3D (Godot)          │
│  • Cuerpo según altura/peso         │
│  • Cara según apariencia            │
...
│  • Renderizar cara 256x256          │
│  • Guardar como PNG                 │
│  • URL: /avatars/piloto_123.png     │
└─────────────────────────────────────┘
```
...
|-------|------|-------------|
| `avatar_url` | VARCHAR | URL de la foto de perfil |
| `altura` | INT | 155-210 cm |
| `peso` | INT | 55-100 kg |
| `pelo_color` | ENUM | negro, castaño, rubio, pelirrojo, gris |
| `pelo_estilo` | ENUM | corto, medio, largo, calvo |
| `ojos_color` | ENUM | marrón, azul, verde, gris, negro |
| `piel_tono` | ENUM | clara, media, morena, oscura |
| `modelo_data` | JSON | Datos del modelo 3D (opcional) |

### Implementación Técnica

| Opción | Pros | Contras |
|--------|------|---------|
| **A: Godot Headless en servidor** | Automático, 3D real | Requiere Godot instalado en servidor, Render puede no permitirlo |
| **B: Generación en cliente** | No necesita servidor especial | Carga inicial lenta, depende del usuario |
| **C: DiceBear (temporal)** | Inmediato, simple, gratis | No es 3D, estilo cartoon |

**Opción A: Godot Headless (Servidor)**
```bash
# Godot puede correr en modo headless para generar imágenes
godot --headless --script generate_avatar.gd --piloto-id 123
```

**Opción B: Generación en Cliente (Primer Visita)**
- Cuando el usuario ve un piloto sin avatar por primera vez
- El navegador genera el avatar con Godot WASM
- Se sube al servidor

**Opción C: Librería JS Alternativa (Más Simple)**
- DiceBear API: `https://api.dicebear.com/7.x/avataaars/svg?seed=piloto123`
- Sin Godot, avatares 2D estilo cartoon
- Inmediato, sin renderizado 3D

### Recomendación Actual

Usar **Opción C (DiceBear)** como solución temporal mientras se implementa Godot:
- Generación instantánea
- Sin carga de Godot en el cliente
- API gratuita con límite generoso
- Avatares consistentes basados en seed (ID del piloto)

```javascript
// Ejemplo de uso
function getAvatarUrl(pilotoId, atributos) {
    const baseUrl = 'https://api.dicebear.com/7.x/avataaars/svg';
    const params = new URLSearchParams({
        seed: pilotoId,
        hairColor: atributos.pelo_color,
        skinColor: atributos.piel_tono,
        eyeColor: atributos.ojos_color
    });
    return `${baseUrl}?${params}`;
}
```

### Cuando Godot esté Listo

1. Exportar Godot con script de generación
2. Servidor ejecuta Godot headless al crear pilotos
3. Guarda imagen en `/public/avatars/`
4. Actualiza `avatar_url` en BD
