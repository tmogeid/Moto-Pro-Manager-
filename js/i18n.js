...
(function() {
    ...
    async function loadTranslations(lang) {
        try {
            ...
            console.log(`[i18n] Traducciones cargadas para ${lang}:`, Object.keys(translations).length, 'claves');
            return true;
        } catch (error) {
    ...
    function t(key) {
        ...
        for (const k of keys) {
            if (value && typeof value === 'object' && k in value) {
                ...
                // Solo mostrar warning si no hay traducciones cargadas
                if (Object.keys(translations).length === 0) {
                    console.warn(`[i18n] Traducciones no cargadas aún para: ${key}`);
                } else {
                    console.warn(`[i18n] Clave no encontrada: ${key} (disponibles: ${Object.keys(translations).slice(0, 5).join(', ')}...)`);
                }
                return key;
            }
        }
    ...
    async function init() {
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', () => {
                ...
                // Disparar evento cuando i18n está listo
    ...
    window.i18n = {
        ...
        setLanguage,
        init,
        // Funciones para verificar si está listo
